...
// --- CREAR ORDEN DE TRABAJO OPTIMIZADO ---
router.post('/', async (req, res) => {
  try {
    ...
    const id = result.insertId;    // **PASO 2: RESPONDER INMEDIATAMENTE - VELOCIDAD MÁXIMA**
    const response = {
      id: id,
      message: 'Work Order created successfully',
      pdfUrl: `/pdfs/workorder_${id}.pdf`
    };
    
    res.status(201).json(response);

    // **PASO 3: GENERAR SOLO PDF MÍNIMO EN BACKGROUND**
    setTimeout(() => {
      try {
        ...
        // PDF ultra-simple para velocidad
        doc.fontSize(20).text('INVOICE - JET SHOP', 50, 50);
        doc.fontSize(12).text(`ID: ${id}`, 50, 100);
        doc.text(`Bill To: ${billToCo || '-'}`, 50, 120);
        doc.text(`Trailer: ${trailer || '-'}`, 50, 140);
        doc.text(`Date: ${formattedDate}`, 50, 160);
        doc.text(`Description: ${description || ''}`, 50, 180, { width: 400 });
        doc.fontSize(14).text(`TOTAL: $${totalLabAndPartsFinal.toFixed(2)}`, 50, 250);
